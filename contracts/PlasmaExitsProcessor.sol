...
//-----------------------------------
    ...
    // // prove that there exist a valid transaction that spends an UTXO that is in exit queue
    // // make it cheap
    ...
    //     require(utxo.spendingTransactionIndex == 0);
    //     require(utxo.pendingExit);
    //     if(utxo.succesfullyWithdrawn) {
    //         revert(); // for now
    //     }
    //     require(!utxo.isLinkedToLimbo);
    ...
    //     uint72 transactionIndex = PlasmaTransactionLibrary.makeTransactionIndex(_plasmaBlockNumber, TX.txNumberInBlock);
    //     utxo.pendingExit = false;
    //     utxo.spendingTransactionIndex = uint160(transactionIndex);
    ...
    public returns (bool success, uint72[] inputsAffected, uint72[] outputsAffected, uint160 transactionIndex) {
        ...
        require(included);
        TX.txNumberInBlock = uint32(txNumber);
        transactionIndex = uint160(PlasmaTransactionLibrary.makeTransactionIndex(_plasmaBlockNumber, TX.txNumberInBlock));
        Transaction storage publishedTransaction = publishedTransactions[transactionIndex];
    ...
    internal returns(Transaction storage _publishedTransaction, uint72[] memory detachedOutputs) {
        if (publishedTransaction.status == PublishedTXstatusWaitingForInputChallenges) {
            ...
            for (scratchSpace[2] = 0; scratchSpace[2] < TX.outputs.length; scratchSpace[2]++) { 
                ...
                scratchSpace[0] = PlasmaTransactionLibrary.makeInputOrOutputIndex(_plasmaBlockNumber, TX.txNumberInBlock, txOutput.outputNumberInTX); // index of created output
                utxo = publishedUTXOs[scratchSpace[0]];
                if (utxo.amountAndOwnerConfirmed) { // this utxo was already once published
